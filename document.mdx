...
- **`with_tax` picks Invoice vs. Bill of Supply.** Send `with_tax: true` for a tax invoice or `false` for a Bill of Supply (tax and item GST fields zeroed). Omit it and Swipe decides for you — from the item tax amounts on create, or by keeping the document's current value on edit. Composition-scheme companies are always forced to `false`.
